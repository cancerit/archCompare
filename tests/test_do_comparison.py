...
class TestClass():
    ...
    def test_cmp_output(self):
        ...
        mystatic_obj.format_results(self.no_ext_cmp_dict_out, self.format_dir_dictA,self.format_dir_dictB, self.tmpoutfile)
        test_cmp_output=fc.ArchCompare(archive_a=self.outfile,archive_b=self.tmpoutfile,json_config=self.t_ignore_ext_json,cmp_type=['diffs'])
        assert self.no_ext_cmp_dict_res == test_cmp_output._do_comparison(self.format_dir_dictA_out,self.format_dir_dictB_out,self.common_files_out),'test_OutputComparison OK'

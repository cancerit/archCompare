...
class ArchCompare(AbstractCompare):
  ...
  def check_input(self):
    ...
    for infile in (self.file_a, self.file_b):
      input_type.append(sm._input_checker(infile))
  ...
  def _format_tar_input(self, file_path):
    ...
    with tarfile.open(file_path, 'r') as tar:
      ...
      if self.cmp_type in ['data', 'checksum']:
        ...
        return sm._process_list_to_dict(path_list, prefix)
  ...
  def _format_file_input(self, file_path):
    ...
    return sm._process_list_to_dict([[file_path, name, ext]], '')
  ...
  def _do_comparison(self, dictA, dictB, common_files):
    ...
    for file_key in common_files:
      ...
      if ext_dict is None:
        ...
        result = sm._do_checksum_comaprison(checksum_type, filea=filea, fileb=fileb)
  ...
  def _run_diff(self, ext_dict, ext, **kwargs):
    ...
    if additional_prm is not None:
      for prm, val in additional_prm.items():
        kwargs[prm] = val
    (stdout, stderr) = sm._run_command(cmd.format(**kwargs))
    if stdout == 'Error':
      return 'Error'
    log.info(('ARGS:', kwargs, 'ERR:', stderr, 'OUT:', stdout))
    if re.search('^.vcf|^.vcf.gz', ext):
      return sm._get_vcf_diff(stdout, exp_out[0])
    else:
      if stderr:
        return
    return 'data'
  ...
  def run_comparison(self):
    ...
    sm._format_results(results, dicta, dictb, self.outfile)
